...
function hand = chanOverviewPlot(species, cleanedEvents, plotStats, ch, win, fs, subject, chan_labels)
...
for rip = 1:size(cleanedEvents.rippleband{ch},1)
    analyticRB(rip,:) = abs((cleanedEvents.rippleband{ch}(rip,:)));
...
RBrange = max(meanRB(center-winZoom:center+winZoom)) - min(meanRB(center-winZoom:center+winZoom));
HFrange = max(meanHF(center-winZoom:center+winZoom)) - min(meanHF(center-winZoom:center+winZoom));

HFscaled = meanHF(center-winZoom:center+winZoom) * (RBrange/HFrange);
ln1 = plot(-winZoom:winZoom,meanRB(center-winZoom:center+winZoom), 'k-'); hold on;
ln1.LineWidth = 2;
ln2 = plot(-winZoom:winZoom,HFscaled - mean(HFscaled), 'r-'); hold on;
ln2.LineWidth = 2;
...
switch species
    case 'human'
        legend([ln1, ln2, ln3], {'rippleband','200+Hz','100-200Hz'});
    case 'rodent'
        legend([ln1, ln2, ln3], {'rippleband','300+Hz','100-200Hz'});
end
